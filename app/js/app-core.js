...
function go(id, resetScroll=true, animate=false){
  ...
  const tabbar = document.getElementById('tabbar');
  const noTab = ['s-login','s-adminlogin','s-admin','s-invite','s-reset-pw','s-church-register','s-dev-stats','s-install','s-report','s-signup','s-admin-login','s-unauthorized','s-admin-dashboard','s-admin-users'];
