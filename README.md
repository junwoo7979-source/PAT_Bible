...
├── README.md                      ← 본 문서
├── app/
│   └── index.html                 ← ⭐ 작동 프로토타입 앱 (브라우저에서 바로 실행)
├── wireframes/
│   └── index.html                 ← 8화면 와이어프레임 (정적 설계도)
├── design/
...
## ⭐ 프로토타입 앱 (app/index.html)

실제 작동하는 단일파일 웹앱. 브라우저로 열면 바로 체험 가능:

- **교회 입장**: 체험 코드 `11111`
- **가족방**: 구성원 달성률 실시간 표시
- **구절 보기**: 주간 암송 구절
- **음성 암송**: Web Speech API STT (1차·2차, 유사도 검증) — 미지원 브라우저는 시뮬레이션
- **타이핑 검증**: 복사·붙여넣기 차단, 한 글자씩 대조(초록/빨강), 100% 일치 시 통과
- **완료·대시보드**: 뱃지 + 개인/가족/교회 통계 (localStorage 저장)
- **설정**: 다크/크림 테마, 큰 글씨, 관대 모드(노년층)
