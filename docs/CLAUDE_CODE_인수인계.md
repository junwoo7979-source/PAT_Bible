...
- 가족방 이름, 가족 대표 이름, 교구와 구역을 입력해 저장한다.
- 값은 `pat_family_profile` localStorage에 `{ roomName, leaderName, parish, district }` 형식으로 저장한다.
...
- 교구 목록은 `1블레싱`, `2블레싱`, `3블레싱` 더미 집계 데이터다.
- 가족방에 입력한 교구와 일치하는 행에 `내 교구` 표시를 붙인다.
- 내 암송 기록이 있으면 현재 가족 교구의 완료 인원에 반영한다.
