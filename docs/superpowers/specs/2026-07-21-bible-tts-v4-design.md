...
부분 제공은 허용한다. 단, 모든 책과 모든 장 항목은 매니페스트에 존재해야 하며 아직 음원이 없는 장은 `available=false`로 명시한다. `available=false` 장은 URL·길이·바이트·체크섬을 생략할 수 있고 재생 버튼을 비활성화해 “음원이 아직 준비되지 않았습니다”라고 표시한다. 장 항목 누락, 장 번호 중복, 실제 장 수 불일치는 매니페스트 전체 오류다.

자동 전환 직후의 다음 장이 `available=false`이면 그 장을 건너뛰지 않는다. 현재 장 종료 처리를 마친 뒤 재생을 정지하고 “다음 장 음원을 준비 중입니다”라고 안내한다. 이후 제공되는 장이나 다음 책으로 자동 이동하지 않는다.
...
1초 시작은 절대 보장이 아니라 성능 목표다. 이미 매니페스트가 로드된 정상 네트워크에서 **해당 음원 캐시 미스 상태**로 사용자 입력부터 실제 `playing` 이벤트까지 1초 이내를 목표로 한다. 앱 콜드 시작이나 느린 네트워크에서는 1초 합격 기준을 적용하지 않고, 1초 안에 로딩 상태가 명확히 표시되는지를 판정한다.
...
stateDiagram-v2
    ...
    ended --> loading: 같은 책의 다음 장이 available=true
    ended --> idle: 다음 장 available=false 또는 책 마지막 장
...
| `ended` | 다음 장 존재 | `available=true` | 새 세션으로 다음 장 `loading`, 기존 배속 유지 |
| `ended` | 다음 장 미제공 | `available=false` | 다음 장을 건너뛰지 않고 준비 중 안내 후 `idle` |
...
- 다음 장은 같은 책 안에서만 자동 재생하고 현재 배속을 유지한다. 다음 장이 `available=false`이면 건너뛰지 않고 정지하며, 마지막 장에서는 완료 안내 후 정지한다.
...
당일 한정 정책의 이유는 현재 읽기표 UI가 과거 날짜를 노출하지 않고 오늘 항목만 보여주기 때문이다. 과거 청취 기록은 데이터 보존과 향후 통계를 위해 내부에 유지하지만, 현재 화면에 표시하지 않고 오늘의 `listenDone` 판정에도 재사용하지 않는다.
...
- `available=false` 장: 해당 장만 비활성화하고 다른 장의 수동 재생은 유지한다. 자동 전환에서는 그 장 앞에서 정지하며 건너뛰지 않는다.
...
아래 시나리오 1~32 전체를 최소 Android Chrome/PWA와 iPhone Safari/PWA에서 각각 수행한다. 잠금화면·이어폰 항목은 OS가 Media Session 동작을 지원하는 범위에서 판정하되, Android에서는 Media Session 주력 검수 항목으로 수행한다.

| 번호 | 조작 | 기대 결과 |
|---|---|---|
| 1 | 매니페스트는 로드됐지만 시편 1편 음원은 캐시에 없는 상태에서, 정상 네트워크로 시편 책 이름을 누른다. | 시편 1편 0초를 로드하고 탭부터 `playing`까지 1초 목표를 충족한다. 콜드 시작·느린 망에서는 1초 안에 로딩 표시가 나타나야 한다. |
...
| 24 | 매니페스트에 존재하지만 `available=false`인 장을 선택한다. | 다른 장의 재생을 오염시키지 않고 해당 장의 준비 중 안내만 표시한다. 장 항목 자체가 누락된 매니페스트는 초기 검증에서 전체 오류로 차단된다. |
...
| 29 | 1.5배속으로 장을 자연 종료시킨다. | 같은 책의 다음 장이 자동 재생되고 1.5배속이 그대로 유지된다. 다음 장이 `available=false`이면 건너뛰지 않고 정지한다. |
| 30 | 책의 마지막 장을 자연 종료시킨다. | 재생이 정지되고 해당 책 듣기 완료 안내가 표시되며 다음 책으로 이동하지 않는다. |
| 31 | iOS에서 다음 장 자동 전환의 `play()`가 거부되는 조건을 만든다. | 다음 장과 이어질 위치를 유지한 `paused` 상태가 되고 “탭하여 계속 듣기” 안내가 표시되며, 탭 한 번으로 재생된다. |
| 32 | 재생 중 기기를 세로에서 가로로, 다시 세로로 회전한다. | 같은 오디오 재생과 현재 위치·배속이 유지되고 플레이어가 중복 생성되지 않는다. |
